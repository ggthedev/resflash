...
if [ -z "${skipfw+1}" ]; then
  ...
  ${BUILDPATH}/05.mkchroot.fw_update 2>&1; then
...
if [ -n "${pkgdir+1}" ]; then
  ...
  if ! chroot ${BUILDPATH}/fs sh -c 'pkg_add -vi -D unsigned /tmp/pkg/*.tgz' \
  >> ${BUILDPATH}/06.mkchroot.pkg_add 2>&1; then
    echo '*** WARNING: Package installation failed due to binary ' \
         'incompatibility, skipping. ***'
  fi
fi
